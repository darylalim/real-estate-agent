    ...
    *,
    ...
    property_search: SubAgent = {
        ...
        "system_prompt": (
            ...
            "3. `search_listings` already returns every field of every match, so do "
            "not re-fetch results with `get_listing`. Use `get_listing` only for a "
            "property that did not come back in your search, such as one the user "
            "named by id.\n"
    ...
    client_liaison: SubAgent = {
        ...
        "system_prompt": (
            ...
            "For drafting, use `save_draft` and nothing else. It is the only correct "
            "way to produce a draft — do not also write the message with `write_file`, "
            "or the reviewer ends up with two divergent copies of one email and has to "
            "guess which is current.\n\n"
            "`save_draft` returns a `.md` copy, a `.eml` the reviewer can open straight "
            "in their mail client, and usually a `mailto_url` for one-click compose. "
            "Surface the `.eml` path and the `mailto_url` when you report back — that "
            "is how the human actually sends it. Pass `to` when you know the address.\n\n"
            "You cannot send, and must not imply that you have. Every draft is a file "
            "on disk awaiting a human. Say so.\n\n"
